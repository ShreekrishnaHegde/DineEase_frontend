...
class CustomerProfileService{
  ...
  Future<CustomerProfileModel> fetchProfile() async {
    final response = await http.get(Uri.parse("$baseUrl/api/customer/$username/profile"));
    print(response.body);
    if (response.statusCode == 200) {
      return CustomerProfileModel.fromJson(jsonDecode(response.body));
    } else {
      throw Exception('Failed to load profile');
    }
  }
  //To update Profile details
  Future<void> updateProfile(CustomerProfileModel profile) async {
    final response = await http.put(
      Uri.parse("$baseUrl/api/customer/$username/profile"),
      headers: {
